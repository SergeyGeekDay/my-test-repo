$(document).ready(function()
{
	...
	{
		var coords = $("#Location8 .locationHv i").offset()
		...
		var percentX = ((posx-mapX) + 7)/$("#mapImg").width()
		var hgh = 18
		var percentY = ((posy-mapY) + hgh) / $("#mapImg").height()
	...
	$("#CheckBtn").click(function()
	{
		...
		for(key in positions)
		{
			...
			var percentX2 = ((posx-mapX) + 7)/$("#mapImg").width()
			var hgh = 24
			var percentY2 = ((posy-mapY) + hgh) / $("#mapImg").height()

			console.log("crd:____"+percentX2 +" "+percentY2)
			...
			{
				if ((item <= percentX2) && (leftX == undefined || item >= leftX))
				{
					leftX = item
				}
				else if ((item>percentX2) && (rightX == undefined || item < rightX))
				{
					rightX = item
				}
			})
			...
			{
				if ((item <= percentY2) && (topY == undefined || item >= topY))
				{
					topY = item
				}
				else if ((item>percentY2) && (bottomY == undefined || item < bottomY))
				{
					bottomY = item
				}
			})
